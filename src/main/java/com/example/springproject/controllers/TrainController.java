...
public class TrainController {
    ...
    String getTrainInfo(Model model, HttpServletRequest request,
        ...
        Train train = null;
        try {
            train = trainService.getTrainById(Long.parseLong(trainId));
    ...
    public String findWithPage(Model model, HttpServletRequest request, String station1,
        ...
        if (date == null || date.equals(""))
            ...
            trains = trainService.getTrainsFilteredByStationsAndDate(station1, station2, date);
            model.addAttribute("date", date);
        }
        if (trains.size() > 5) {
            trains = GeneralUtility.paginateList(trains, page);
